#ifndef __HOUSE_NETWORK_H
#define __HOUSE_NETWORK_H
...
struct _md5_data {
    uint8_t size;
    uint8_t data[36]; // usually 16
};

typedef struct _ext_data 
{
	uint8_t code; // 2: response, it's always 2 in house_dial route, but in the request pkt, it's maybe 4(failure), 3(success) 
	...
	union
	{
		...
		uint8_t			  id_data[37]; // Identity num
